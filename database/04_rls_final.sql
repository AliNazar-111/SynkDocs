-- ============================================
-- SYNKDOCS RLS POLICIES - FINAL VERSION
-- ============================================
-- Uses SECURITY DEFINER functions in public schema
...
-- These run without RLS checks, breaking circular refs
-- ============================================

-- Temporarily disable RLS to create functions
ALTER TABLE documents DISABLE ROW LEVEL SECURITY;
ALTER TABLE document_collaborators DISABLE ROW LEVEL SECURITY;

-- Function: Check if current user owns document
...
-- Function: Check if current user is collaborator
CREATE OR REPLACE FUNCTION public.user_is_collaborator(doc_id UUID)
RETURNS BOOLEAN AS $$
BEGIN
  RETURN EXISTS (
...
-- Function: Check if current user has editor access
CREATE OR REPLACE FUNCTION public.user_is_editor(doc_id UUID)
RETURNS BOOLEAN AS $$
BEGIN
  RETURN EXISTS (
    SELECT 1 FROM public.document_collaborators
...
CREATE POLICY "collaborators_select_policy"
  ...
  FOR SELECT
  USING (
    public.user_owns_document(document_id)
    OR user_id = auth.uid()
  );
...
CREATE POLICY "comments_select_policy"
  ...
  USING (
    ...
    AND (
      public.user_owns_document(document_id)
      OR public.user_is_collaborator(document_id)
    )
  );
...
CREATE POLICY "comments_insert_policy"
  ...
  WITH CHECK (
    ...
    AND (
      public.user_owns_document(document_id)
      OR public.user_is_editor(document_id)
    )
  );
...
-- Functions are in public schema (allowed)
-- No circular references!

SELECT 'RLS Policies installed successfully!' as status;
